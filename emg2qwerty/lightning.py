...
class CNNTransformerCTCModule(pl.LightningModule):
    """CTC module with CNN + Transformer encoder (replaces TDS). Same front-end
    (SpectrogramNorm, MultiBandRotationInvariantMLP) then CNNTransformerEncoder."""
...
class CNNBiLSTMCTCModule(pl.LightningModule):
    """Lighter CTC module: CNN + BiLSTM encoder with CR-CTC. Same front-end
    (SpectrogramNorm, MultiBandRotationInvariantMLP) then CNNBiLSTMEncoder."""
    ...
    def __init__(
        ...
        cr_ctc_consistency_weight: float = 0.0,
        cr_ctc_entropy_weight: float = 0.01,
        optimizer: DictConfig | None = None,
        lr_scheduler: DictConfig | None = None,
        decoder: DictConfig | None = None,
    ) -> None:
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        x = self.front_end(inputs)  # (T, N, num_features)
        x = self.encoder(x)         # (T, N, 2*lstm_hidden)
        return self.head(x)         # (T, N, num_classes)
